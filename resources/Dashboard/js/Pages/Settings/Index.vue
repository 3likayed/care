...
<template>
  <SectionMain>
    ...
    <SectionTitleLineWithButton :icon="mdiLockAlertOutline" :title="__('settings')" main>
    ...
    <CardBox is-form @submit.prevent="submit">
      ...
      <div v-if="step===0">
        <FormWithLabelField :label="__('site_name')">
          <FormField v-for="locale in locales" :errors="form.errors[`name.${locale}`]"
                     :label="__(locale)"
          >
            <FormControl
        ...
        <FormWithLabelField :label="__('site_title')">
          <FormField v-for="locale in locales" :errors="form.errors[`title.${locale}`]"
                     :label="__(locale)"
          >
            <FormControl
        ...
        <FormWithLabelField :label="__('site_description')">
          <FormField v-for="locale in locales" :errors="form.errors[`description.${locale}`]"
                     :label="__(locale)"
          >
            <FormControl
        ...
        <FormWithLabelField :cols="{sm:1 , lg:2 }" :label="__('social_media')">
          <FormField :errors="form.errors[`social.facebook`]"
                     :label="__('facebook')"
          >
            <FormControl
          ...
          <FormField :errors="form.errors[`social.twitter`]"
                     :label="__('twitter')"
          >
            <FormControl
          ...
          <FormField :errors="form.errors[`social.instagram`]"
                     :label="__('instagram')"
          >
            <FormControl
          ...
          <FormField :errors="form.errors[`social.linkedin`]"
                     :label="__('linkedin')"
          >
            <FormControl
        ...
        <div class="grid lg:grid-cols-3 gap-3 mt-10">
          <FormWithLabelField :button="{color:'success' , icon:mdiPlusCircle }"
                              :cols="{sm:1 , lg:1 }" :errors="form.errors.email"
                              :label="__('email')"
                              @button-click="addField('email')">
            <FormField v-for="(email,key) in form.email"
                       :key="key"
          ...
          <FormWithLabelField :button="{color:'success' , icon:mdiPlusCircle }"
                              :cols="{sm:1 , lg:1 }" :errors="form.errors.phone"
                              :label="__('phone')"
                              @button-click="addField('phone')">
            <FormField v-for="(phone,key) in form.phone"
                       :key="key"
          ...
          <FormWithLabelField :button="{color:'success' , icon:mdiPlusCircle }"
            ...
            <FormField v-for="(whatsapp,key) in form.whatsapp"
                       :key="key"
        ...
        <FormWithLabelField :cols="{lg:1}" :label="__('site_location')">
          <GoogleMap v-model="form.location" :center="{lat:form.location.lat , lng:form.location.lng}"/>
        </FormWithLabelField>
      </div>
      <div v-if="step===1">
      ...
      <template #footer>
        <BaseButtons>
          <BaseButton :label="__('continue')" color="info" type="submit"/>
        </BaseButtons>
      </template>
    </CardBox>
  </SectionMain>
