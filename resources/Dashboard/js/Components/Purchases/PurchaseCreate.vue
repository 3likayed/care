...
const appendProduct = (stockable_id) => {
    let product = collect(props[pluralize(type.value)]).firstWhere('id', '=', stockable_id)
    form.stockables.push({id: product.id, name: product.name, total: 0, type: type.value});
