<template>
    ...
    <SectionTitleLineWithButton :icon="mdiLockAlertOutline" :title="__('reservation-types')" main>
        <slot name="create">
    ...
    <DynamicSearch v-if="hasSearch" :fields="[{name:'name'}]" model="reservation-types"/>
    <CardBox has-table>
        <BaseTable
        ...
        >
            <tr v-for="(item,key) in items" class="rtl:flex-row-reverse">
                ...
                <td :data-label="__('options')">
                    <TableOptions :has-show="false" :item="item" model="reservation-types" @edit="edited=item">
                        <template #edit>
                            <EditReservationType :data="edited"/>
                        </template>
                    </TableOptions>
                </td>
            </tr>
...
import EditReservationType from "./ReservationTypeEdit.vue";
import CreateReservationType from "./ReservationTypeCreate.vue";
