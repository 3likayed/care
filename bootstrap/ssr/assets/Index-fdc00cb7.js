import { inject, ref, mergeProps, unref, isRef, withCtx, createVNode, openBlock, createBlock, Fragment, renderList, withDirectives, vShow, useSSRContext, provide, toDisplayString } from "vue";
import { ssrRenderComponent, ssrRenderStyle, ssrRenderList, ssrInterpolate, ssrRenderAttr } from "vue/server-renderer";
import { mdiAccountStarOutline, mdiPlusCircle } from "@mdi/js";
import { _ as _sfc_main$f } from "./TableOptions-a9345eb3.js";
import { _ as _sfc_main$9, a as _sfc_main$b } from "./SectionTitleLineWithButton-9da2ae80.js";
import { _ as _sfc_main$2, a as _sfc_main$e, b as _sfc_main$h } from "./ActionConfirmComponent-f1d725a5.js";
import { g as __, _ as _sfc_main$6, a as _sfc_main$c } from "../app.js";
import { _ as _sfc_main$5 } from "./FormField-a416fc4e.js";
import { usePage, useForm, router } from "@inertiajs/vue3";
import { _ as _sfc_main$4 } from "./FormWithLabelField-2c980605.js";
import { _ as _sfc_main$3 } from "./StepsComponent-202818d9.js";
import { _ as _sfc_main$7 } from "./MetaDataComponent-cac40315.js";
import { _ as _sfc_main$8 } from "./ShowMediaComponent-9a2f83fc.js";
import { _ as _sfc_main$d } from "./CardBox-b517a69f.js";
import { _ as _sfc_main$g } from "./EditPage-45435585.js";
import { _ as _sfc_main$a } from "./BreadCrumb-4683e004.js";
import "pinia";
import "ziggy-js";
/* empty css                                 */import "axios";
import "@tinymce/tinymce-vue";
import "sweetalert2";
/* empty css                      */import "./laravel-vite-plugin-af3ef812.js";
import "@fawmi/vue-google-maps";
import "chart.js/helpers";
const _sfc_main$1 = {
  __name: "CreatePage",
  __ssrInlineRender: true,
  setup(__props) {
    let locales = usePage().props.locales;
    let showCreatePage = inject("showCreatePage");
    let form = useForm({
      title: {},
      content: {},
      priority: null,
      image: null,
      meta: {
        title: {},
        description: {},
        keywords: {},
        slug: {}
      }
    });
    let steps = ref([__("main_data"), __("meta"), __("media")]);
    let step = ref(0);
    function submit() {
      form.post(route("dashboard.pages.store"), {
        preserveScroll: true,
        preserveState: (page) => Object.keys(page.props.errors).length,
        onSuccess: () => {
          showCreatePage.value = false;
        }
      });
    }
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$2, mergeProps({
        "button-label": unref(__)("create"),
        "has-cancel": true,
        hasErrors: unref(form).hasErrors,
        "is-form": true,
        "model-value": unref(showCreatePage),
        title: unref(__)("create_field", { field: "page" }),
        onCancel: ($event) => isRef(showCreatePage) ? showCreatePage.value = false : showCreatePage = false,
        onConfirm: submit
      }, _attrs), {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(ssrRenderComponent(_sfc_main$3, {
              modelValue: unref(step),
              "onUpdate:modelValue": ($event) => isRef(step) ? step.value = $event : step = $event,
              steps: unref(steps)
            }, null, _parent2, _scopeId));
            _push2(`<section style="${ssrRenderStyle(unref(step) === 0 ? null : { display: "none" })}"${_scopeId}>`);
            _push2(ssrRenderComponent(_sfc_main$4, {
              label: unref(__)("title")
            }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`<!--[-->`);
                  ssrRenderList(unref(locales), (locale) => {
                    _push3(ssrRenderComponent(_sfc_main$5, {
                      errors: unref(form).errors[`title.${locale}`],
                      label: unref(__)(locale)
                    }, {
                      default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                        if (_push4) {
                          _push4(ssrRenderComponent(_sfc_main$6, {
                            modelValue: unref(form).title[locale],
                            "onUpdate:modelValue": ($event) => unref(form).title[locale] = $event,
                            autocomplete: `title[${locale}]`,
                            name: `title[${locale}]`,
                            required: ""
                          }, null, _parent4, _scopeId3));
                        } else {
                          return [
                            createVNode(_sfc_main$6, {
                              modelValue: unref(form).title[locale],
                              "onUpdate:modelValue": ($event) => unref(form).title[locale] = $event,
                              autocomplete: `title[${locale}]`,
                              name: `title[${locale}]`,
                              required: ""
                            }, null, 8, ["modelValue", "onUpdate:modelValue", "autocomplete", "name"])
                          ];
                        }
                      }),
                      _: 2
                    }, _parent3, _scopeId2));
                  });
                  _push3(`<!--]-->`);
                } else {
                  return [
                    (openBlock(true), createBlock(Fragment, null, renderList(unref(locales), (locale) => {
                      return openBlock(), createBlock(_sfc_main$5, {
                        errors: unref(form).errors[`title.${locale}`],
                        label: unref(__)(locale)
                      }, {
                        default: withCtx(() => [
                          createVNode(_sfc_main$6, {
                            modelValue: unref(form).title[locale],
                            "onUpdate:modelValue": ($event) => unref(form).title[locale] = $event,
                            autocomplete: `title[${locale}]`,
                            name: `title[${locale}]`,
                            required: ""
                          }, null, 8, ["modelValue", "onUpdate:modelValue", "autocomplete", "name"])
                        ]),
                        _: 2
                      }, 1032, ["errors", "label"]);
                    }), 256))
                  ];
                }
              }),
              _: 1
            }, _parent2, _scopeId));
            _push2(ssrRenderComponent(_sfc_main$4, {
              cols: { lg: 1 },
              label: unref(__)("content")
            }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`<!--[-->`);
                  ssrRenderList(unref(locales), (locale) => {
                    _push3(ssrRenderComponent(_sfc_main$5, {
                      errors: unref(form).errors[`content.${locale}`],
                      label: unref(__)(locale)
                    }, {
                      default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                        if (_push4) {
                          _push4(ssrRenderComponent(_sfc_main$6, {
                            modelValue: unref(form).content[locale],
                            "onUpdate:modelValue": ($event) => unref(form).content[locale] = $event,
                            autocomplete: `content[${locale}]`,
                            dir: locale === "ar" ? "rtl" : "ltr",
                            name: `content[${locale}]`,
                            required: "",
                            type: "editor"
                          }, null, _parent4, _scopeId3));
                        } else {
                          return [
                            createVNode(_sfc_main$6, {
                              modelValue: unref(form).content[locale],
                              "onUpdate:modelValue": ($event) => unref(form).content[locale] = $event,
                              autocomplete: `content[${locale}]`,
                              dir: locale === "ar" ? "rtl" : "ltr",
                              name: `content[${locale}]`,
                              required: "",
                              type: "editor"
                            }, null, 8, ["modelValue", "onUpdate:modelValue", "autocomplete", "dir", "name"])
                          ];
                        }
                      }),
                      _: 2
                    }, _parent3, _scopeId2));
                  });
                  _push3(`<!--]-->`);
                } else {
                  return [
                    (openBlock(true), createBlock(Fragment, null, renderList(unref(locales), (locale) => {
                      return openBlock(), createBlock(_sfc_main$5, {
                        errors: unref(form).errors[`content.${locale}`],
                        label: unref(__)(locale)
                      }, {
                        default: withCtx(() => [
                          createVNode(_sfc_main$6, {
                            modelValue: unref(form).content[locale],
                            "onUpdate:modelValue": ($event) => unref(form).content[locale] = $event,
                            autocomplete: `content[${locale}]`,
                            dir: locale === "ar" ? "rtl" : "ltr",
                            name: `content[${locale}]`,
                            required: "",
                            type: "editor"
                          }, null, 8, ["modelValue", "onUpdate:modelValue", "autocomplete", "dir", "name"])
                        ]),
                        _: 2
                      }, 1032, ["errors", "label"]);
                    }), 256))
                  ];
                }
              }),
              _: 1
            }, _parent2, _scopeId));
            _push2(ssrRenderComponent(_sfc_main$4, {
              cols: { lg: 1 },
              label: unref(__)("priority")
            }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(ssrRenderComponent(_sfc_main$5, {
                    errors: unref(form).errors.priority
                  }, {
                    default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        _push4(ssrRenderComponent(_sfc_main$6, {
                          modelValue: unref(form).priority,
                          "onUpdate:modelValue": ($event) => unref(form).priority = $event,
                          autocomplete: "priority",
                          name: "priority",
                          required: "",
                          type: "number"
                        }, null, _parent4, _scopeId3));
                      } else {
                        return [
                          createVNode(_sfc_main$6, {
                            modelValue: unref(form).priority,
                            "onUpdate:modelValue": ($event) => unref(form).priority = $event,
                            autocomplete: "priority",
                            name: "priority",
                            required: "",
                            type: "number"
                          }, null, 8, ["modelValue", "onUpdate:modelValue"])
                        ];
                      }
                    }),
                    _: 1
                  }, _parent3, _scopeId2));
                } else {
                  return [
                    createVNode(_sfc_main$5, {
                      errors: unref(form).errors.priority
                    }, {
                      default: withCtx(() => [
                        createVNode(_sfc_main$6, {
                          modelValue: unref(form).priority,
                          "onUpdate:modelValue": ($event) => unref(form).priority = $event,
                          autocomplete: "priority",
                          name: "priority",
                          required: "",
                          type: "number"
                        }, null, 8, ["modelValue", "onUpdate:modelValue"])
                      ]),
                      _: 1
                    }, 8, ["errors"])
                  ];
                }
              }),
              _: 1
            }, _parent2, _scopeId));
            _push2(`</section><section style="${ssrRenderStyle(unref(step) === 1 ? null : { display: "none" })}"${_scopeId}>`);
            _push2(ssrRenderComponent(_sfc_main$7, { "model-value": unref(form) }, null, _parent2, _scopeId));
            _push2(`</section><section style="${ssrRenderStyle(unref(step) === 2 ? null : { display: "none" })}"${_scopeId}>`);
            _push2(ssrRenderComponent(_sfc_main$8, {
              modelValue: unref(form).image,
              "onUpdate:modelValue": ($event) => unref(form).image = $event,
              name: "image"
            }, null, _parent2, _scopeId));
            _push2(`</section>`);
          } else {
            return [
              createVNode(_sfc_main$3, {
                modelValue: unref(step),
                "onUpdate:modelValue": ($event) => isRef(step) ? step.value = $event : step = $event,
                steps: unref(steps)
              }, null, 8, ["modelValue", "onUpdate:modelValue", "steps"]),
              withDirectives(createVNode("section", null, [
                createVNode(_sfc_main$4, {
                  label: unref(__)("title")
                }, {
                  default: withCtx(() => [
                    (openBlock(true), createBlock(Fragment, null, renderList(unref(locales), (locale) => {
                      return openBlock(), createBlock(_sfc_main$5, {
                        errors: unref(form).errors[`title.${locale}`],
                        label: unref(__)(locale)
                      }, {
                        default: withCtx(() => [
                          createVNode(_sfc_main$6, {
                            modelValue: unref(form).title[locale],
                            "onUpdate:modelValue": ($event) => unref(form).title[locale] = $event,
                            autocomplete: `title[${locale}]`,
                            name: `title[${locale}]`,
                            required: ""
                          }, null, 8, ["modelValue", "onUpdate:modelValue", "autocomplete", "name"])
                        ]),
                        _: 2
                      }, 1032, ["errors", "label"]);
                    }), 256))
                  ]),
                  _: 1
                }, 8, ["label"]),
                createVNode(_sfc_main$4, {
                  cols: { lg: 1 },
                  label: unref(__)("content")
                }, {
                  default: withCtx(() => [
                    (openBlock(true), createBlock(Fragment, null, renderList(unref(locales), (locale) => {
                      return openBlock(), createBlock(_sfc_main$5, {
                        errors: unref(form).errors[`content.${locale}`],
                        label: unref(__)(locale)
                      }, {
                        default: withCtx(() => [
                          createVNode(_sfc_main$6, {
                            modelValue: unref(form).content[locale],
                            "onUpdate:modelValue": ($event) => unref(form).content[locale] = $event,
                            autocomplete: `content[${locale}]`,
                            dir: locale === "ar" ? "rtl" : "ltr",
                            name: `content[${locale}]`,
                            required: "",
                            type: "editor"
                          }, null, 8, ["modelValue", "onUpdate:modelValue", "autocomplete", "dir", "name"])
                        ]),
                        _: 2
                      }, 1032, ["errors", "label"]);
                    }), 256))
                  ]),
                  _: 1
                }, 8, ["label"]),
                createVNode(_sfc_main$4, {
                  cols: { lg: 1 },
                  label: unref(__)("priority")
                }, {
                  default: withCtx(() => [
                    createVNode(_sfc_main$5, {
                      errors: unref(form).errors.priority
                    }, {
                      default: withCtx(() => [
                        createVNode(_sfc_main$6, {
                          modelValue: unref(form).priority,
                          "onUpdate:modelValue": ($event) => unref(form).priority = $event,
                          autocomplete: "priority",
                          name: "priority",
                          required: "",
                          type: "number"
                        }, null, 8, ["modelValue", "onUpdate:modelValue"])
                      ]),
                      _: 1
                    }, 8, ["errors"])
                  ]),
                  _: 1
                }, 8, ["label"])
              ], 512), [
                [vShow, unref(step) === 0]
              ]),
              withDirectives(createVNode("section", null, [
                createVNode(_sfc_main$7, { "model-value": unref(form) }, null, 8, ["model-value"])
              ], 512), [
                [vShow, unref(step) === 1]
              ]),
              withDirectives(createVNode("section", null, [
                createVNode(_sfc_main$8, {
                  modelValue: unref(form).image,
                  "onUpdate:modelValue": ($event) => unref(form).image = $event,
                  name: "image"
                }, null, 8, ["modelValue", "onUpdate:modelValue"])
              ], 512), [
                [vShow, unref(step) === 2]
              ])
            ];
          }
        }),
        _: 1
      }, _parent));
    };
  }
};
const _sfc_setup$1 = _sfc_main$1.setup;
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/Dashboard/js/Components/Pages/CreatePage.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const _sfc_main = {
  __name: "Index",
  __ssrInlineRender: true,
  setup(__props) {
    let showCreatePage = ref(false);
    let showEditPage = ref(false);
    let actionConfirm = ref(false);
    let pages = usePage().props.pages.data;
    let editedPage = ref({});
    let deletedPage = ref(null);
    provide("showEditPage", showEditPage);
    provide("showCreatePage", showCreatePage);
    provide("actionConfirm", actionConfirm);
    usePage().props.locales;
    function edit(page) {
      editedPage.value = page;
      showEditPage.value = true;
    }
    function destroy(page) {
      actionConfirm.value = true;
      deletedPage = page.id;
    }
    function destroyConfirm() {
      router.delete(route("dashboard.pages.destroy", deletedPage), {
        preserveState: (page) => Object.keys(page.props.errors).length,
        preserveScroll: true
      });
    }
    function show(page) {
      router.visit(route("dashboard.pages.show", page));
    }
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[-->`);
      _push(ssrRenderComponent(_sfc_main$9, null, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(ssrRenderComponent(_sfc_main$a, {
              items: [{ name: _ctx.__("pages"), href: _ctx.route("dashboard.pages.index") }]
            }, null, _parent2, _scopeId));
            _push2(ssrRenderComponent(_sfc_main$b, {
              icon: unref(mdiAccountStarOutline),
              title: _ctx.__("pages"),
              main: ""
            }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(ssrRenderComponent(_sfc_main$c, {
                    icon: unref(mdiPlusCircle),
                    color: "success",
                    onClick: ($event) => isRef(showCreatePage) ? showCreatePage.value = true : showCreatePage = true
                  }, null, _parent3, _scopeId2));
                } else {
                  return [
                    createVNode(_sfc_main$c, {
                      icon: unref(mdiPlusCircle),
                      color: "success",
                      onClick: ($event) => isRef(showCreatePage) ? showCreatePage.value = true : showCreatePage = true
                    }, null, 8, ["icon", "onClick"])
                  ];
                }
              }),
              _: 1
            }, _parent2, _scopeId));
            _push2(ssrRenderComponent(_sfc_main$d, { "has-table": "" }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(ssrRenderComponent(_sfc_main$e, {
                    headers: ["#", _ctx.__("name"), _ctx.__("priority"), _ctx.__("created_at")]
                  }, {
                    default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        _push4(`<!--[-->`);
                        ssrRenderList(unref(pages), (page, key) => {
                          _push4(`<tr class="rtl:flex-row-reverse"${_scopeId3}><td data-label="# "${_scopeId3}>${ssrInterpolate(key + 1)}</td><td${ssrRenderAttr("data-label", _ctx.__("title"))} class="border-b-0 before:hidden justify-center mx-auto"${_scopeId3}>${ssrInterpolate(page.title)}</td><td${ssrRenderAttr("data-label", _ctx.__("priority"))}${_scopeId3}>${ssrInterpolate(page.priority)}</td><td${ssrRenderAttr("data-label", _ctx.__("created_at"))}${_scopeId3}>${ssrInterpolate(page.created_at)}</td><td${ssrRenderAttr("data-label", _ctx.__("options"))}${_scopeId3}>`);
                          _push4(ssrRenderComponent(_sfc_main$f, {
                            model: "pages",
                            onShow: ($event) => show(page),
                            onDestroy: ($event) => destroy(page),
                            onEdit: ($event) => edit(page)
                          }, null, _parent4, _scopeId3));
                          _push4(`</td></tr>`);
                        });
                        _push4(`<!--]-->`);
                      } else {
                        return [
                          (openBlock(true), createBlock(Fragment, null, renderList(unref(pages), (page, key) => {
                            return openBlock(), createBlock("tr", {
                              key,
                              class: "rtl:flex-row-reverse"
                            }, [
                              createVNode("td", { "data-label": "# " }, toDisplayString(key + 1), 1),
                              createVNode("td", {
                                "data-label": _ctx.__("title"),
                                class: "border-b-0 before:hidden justify-center mx-auto"
                              }, toDisplayString(page.title), 9, ["data-label"]),
                              createVNode("td", {
                                "data-label": _ctx.__("priority")
                              }, toDisplayString(page.priority), 9, ["data-label"]),
                              createVNode("td", {
                                "data-label": _ctx.__("created_at")
                              }, toDisplayString(page.created_at), 9, ["data-label"]),
                              createVNode("td", {
                                "data-label": _ctx.__("options")
                              }, [
                                createVNode(_sfc_main$f, {
                                  model: "pages",
                                  onShow: ($event) => show(page),
                                  onDestroy: ($event) => destroy(page),
                                  onEdit: ($event) => edit(page)
                                }, null, 8, ["onShow", "onDestroy", "onEdit"])
                              ], 8, ["data-label"])
                            ]);
                          }), 128))
                        ];
                      }
                    }),
                    _: 1
                  }, _parent3, _scopeId2));
                } else {
                  return [
                    createVNode(_sfc_main$e, {
                      headers: ["#", _ctx.__("name"), _ctx.__("priority"), _ctx.__("created_at")]
                    }, {
                      default: withCtx(() => [
                        (openBlock(true), createBlock(Fragment, null, renderList(unref(pages), (page, key) => {
                          return openBlock(), createBlock("tr", {
                            key,
                            class: "rtl:flex-row-reverse"
                          }, [
                            createVNode("td", { "data-label": "# " }, toDisplayString(key + 1), 1),
                            createVNode("td", {
                              "data-label": _ctx.__("title"),
                              class: "border-b-0 before:hidden justify-center mx-auto"
                            }, toDisplayString(page.title), 9, ["data-label"]),
                            createVNode("td", {
                              "data-label": _ctx.__("priority")
                            }, toDisplayString(page.priority), 9, ["data-label"]),
                            createVNode("td", {
                              "data-label": _ctx.__("created_at")
                            }, toDisplayString(page.created_at), 9, ["data-label"]),
                            createVNode("td", {
                              "data-label": _ctx.__("options")
                            }, [
                              createVNode(_sfc_main$f, {
                                model: "pages",
                                onShow: ($event) => show(page),
                                onDestroy: ($event) => destroy(page),
                                onEdit: ($event) => edit(page)
                              }, null, 8, ["onShow", "onDestroy", "onEdit"])
                            ], 8, ["data-label"])
                          ]);
                        }), 128))
                      ]),
                      _: 1
                    }, 8, ["headers"])
                  ];
                }
              }),
              _: 1
            }, _parent2, _scopeId));
          } else {
            return [
              createVNode(_sfc_main$a, {
                items: [{ name: _ctx.__("pages"), href: _ctx.route("dashboard.pages.index") }]
              }, null, 8, ["items"]),
              createVNode(_sfc_main$b, {
                icon: unref(mdiAccountStarOutline),
                title: _ctx.__("pages"),
                main: ""
              }, {
                default: withCtx(() => [
                  createVNode(_sfc_main$c, {
                    icon: unref(mdiPlusCircle),
                    color: "success",
                    onClick: ($event) => isRef(showCreatePage) ? showCreatePage.value = true : showCreatePage = true
                  }, null, 8, ["icon", "onClick"])
                ]),
                _: 1
              }, 8, ["icon", "title"]),
              createVNode(_sfc_main$d, { "has-table": "" }, {
                default: withCtx(() => [
                  createVNode(_sfc_main$e, {
                    headers: ["#", _ctx.__("name"), _ctx.__("priority"), _ctx.__("created_at")]
                  }, {
                    default: withCtx(() => [
                      (openBlock(true), createBlock(Fragment, null, renderList(unref(pages), (page, key) => {
                        return openBlock(), createBlock("tr", {
                          key,
                          class: "rtl:flex-row-reverse"
                        }, [
                          createVNode("td", { "data-label": "# " }, toDisplayString(key + 1), 1),
                          createVNode("td", {
                            "data-label": _ctx.__("title"),
                            class: "border-b-0 before:hidden justify-center mx-auto"
                          }, toDisplayString(page.title), 9, ["data-label"]),
                          createVNode("td", {
                            "data-label": _ctx.__("priority")
                          }, toDisplayString(page.priority), 9, ["data-label"]),
                          createVNode("td", {
                            "data-label": _ctx.__("created_at")
                          }, toDisplayString(page.created_at), 9, ["data-label"]),
                          createVNode("td", {
                            "data-label": _ctx.__("options")
                          }, [
                            createVNode(_sfc_main$f, {
                              model: "pages",
                              onShow: ($event) => show(page),
                              onDestroy: ($event) => destroy(page),
                              onEdit: ($event) => edit(page)
                            }, null, 8, ["onShow", "onDestroy", "onEdit"])
                          ], 8, ["data-label"])
                        ]);
                      }), 128))
                    ]),
                    _: 1
                  }, 8, ["headers"])
                ]),
                _: 1
              })
            ];
          }
        }),
        _: 1
      }, _parent));
      if (unref(showEditPage)) {
        _push(ssrRenderComponent(_sfc_main$g, {
          editedPage: unref(editedPage),
          "is-modal": ""
        }, null, _parent));
      } else {
        _push(`<!---->`);
      }
      if (unref(showCreatePage)) {
        _push(ssrRenderComponent(_sfc_main$1, null, null, _parent));
      } else {
        _push(`<!---->`);
      }
      _push(ssrRenderComponent(_sfc_main$h, { onConfirm: destroyConfirm }, null, _parent));
      _push(`<!--]-->`);
    };
  }
};
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/Dashboard/js/Pages/Pages/Index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
export {
  _sfc_main as default
};
